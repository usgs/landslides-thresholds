...
program tsthresh
implicit none
integer:: count_rf_sort, count_sli_rf_sort,num_incr,u(4),i,j,k,num_pairs,line_ctr,itsmax,dec_count
integer, allocatable:: count_rf_incr_excd(:),count_sli_incr_excd(:)
real, allocatable:: rf_sort(:), sli_rf_sort(:), rf_incr(:),duration(:),ts(:), tp_rate(:), fp_rate(:)
real:: rfmax, rfmin, tsmax, Y, tp, tn, fp, fn, temp, fac_10, auc
character (len=256):: rf_sort_fil, rf_sli_sort_fil,junk
...
rewind(u(1))
allocate(duration(num_pairs))
duration=0.;line_ctr=1
do k=1, num_pairs
    read(u(1),*,err=110, end=20) duration(k)
    line_ctr=line_ctr+1
...
!    inquire(file=trim(rf_sli_sort_fil ),exist=ans2)
    ...
    50 continue
    rewind(u(2))
    write(*,*) 'count_rf_sort ',count_rf_sort
...
! Determine multiplier for fixed decimal input.
    ...
    do
        if(abs(fac_10*(rfmax-rfmin)-int(fac_10*(rfmax-rfmin)))>0.099 .or. fac_10<100.) then
            fac_10=10.0*fac_10
            write(*,*) fac_10, fac_10*(rfmax-rfmin), abs(fac_10*(rfmax-rfmin)-int(fac_10*(rfmax-rfmin)))
        else
            exit
        end if
    end do
    ...
    write(*,*) 'rfmax, rfmin, num_incr, fac_10 ',rfmax, rfmin, num_incr, fac_10
    do
    ...
    60 continue
    rewind(u(3))
    write(*,*) 'count_sli_rf_sort ',count_sli_rf_sort
    ...
    auc=0.
    write(*,*)  'rf_incr(i), tp, Y (=tp+fp), fn, tn'
    do i=1,num_incr
        ...
        tp_rate(i)=tp/(tp+fn)
        fp_rate(i)=fp/(fp+tn)
        write(*,*)  rf_incr(i), cm, tp, cm, Y, cm, fn, cm, tn
        ts(i)=tp/(Y+fn)
        if(ts(i)>tsmax) then
            tsmax=ts(i)
            itsmax=i
...
!    tsmax=maxval(ts)
    write(*,*) 'Duration, tsmax, rf_incr(itsmax) : ', duration(k), tsmax, rf_incr(itsmax)
    write(*,*) 'Area under ROC curve (AUC): ', auc
    write(*,*) 'Cumulative rainfall' , cm,'TS ', cm, 'TP rate ', cm, 'FP rate'
    do i=1,num_incr                 
        write(*,*) rf_incr(i), cm, ts(i), cm, tp_rate(i), cm, fp_rate(i)
    end do
...
150 continue
    write (*,*) '*** Error reading numeric value frm data file in tsthresh ***'
    write (*,*) '--> ',trim(rf_sort_fil)
...
write(*,*) 'Last duration might have been skipped'
